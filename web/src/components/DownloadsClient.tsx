...
const TABS: { key: Section; label: string; icon: string; adminOnly?: boolean }[] = [
  { key: "extension", label: "Chrome Extension", icon: "🧩" },
  { key: "connector", label: "Claude Connector",  icon: "🤖" },
  { key: "api",       label: "API Reference",     icon: "📡" },
  { key: "admin",     label: "Admin Guide",        icon: "⚙️", adminOnly: true },
];
...
function H2({ children }: { children: React.ReactNode }) {
  return <h2 className="mt-8 mb-2 text-base font-semibold text-white/90">{children}</h2>;
}
...
function ExtensionGuide() {
  return (
    <div>
      <H2>🧩 Chrome Extension — How it works</H2>
...
function ConnectorGuide() {
  return (
    <div>
      <H2>🤖 Claude Connector — Developer Guide</H2>
  ...
  "apiKey": "llr_live_xxxxxxxxxxxxxxxxxxxxxxxx"
      ...
      <P>Open VS Code → Claude chat → click ⚙️ (system prompt) → paste:</P>
...
function AdminGuide() {
  return (
    <div>
      ...
      <P>Project page → <strong>📋 Kanban</strong> tab. Drag cards between columns to update status instantly. You can also click any card to open the full bug detail.</P>
    </div>
  );
}
...
export function DownloadsClient({ role }: { role: string }) {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-semibold">📥 Downloads & Guides</h1>
